    ...
    <div class="son">
        ...
        <a-comment datetime="2025-11-14" align="right">
            <template #author>
                ...
                回复：
                <router-link target="_blank" :to="{ path: `/user/${pops.data.userid}` }">{{
                    `@${pops.data.name}` }}</router-link>
            ...
            <template #avatar>
                ...
                <router-link target="_blank" :to="{ path: `/user/${pops.data.usercoment.id}` }">
                    <a-avatar>
...
const pops = defineProps(["data", "id"])
